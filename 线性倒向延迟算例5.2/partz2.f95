...
!-llapck -lblas
	implicit none
	private
	public::finalvalue, gauss_hermite, thelengthofx, bubble_sort, newtoninterpl, leastsquare,  li52
...
contains
	...
	!-llapck -lblas
		...
		real(kind=8), dimension(:,:), allocatable::y, z, ztemp 
		real(kind=8), dimension(:),allocatable:: t, x
		real(kind=8):: timesteplength, xsteplength, yb, zb
	
! 	
		integer::  i, timeindex, xindex, xstep, xrange, k
	...
	! x space 
		...
		allocate(ztemp(0:timestep,-xstep:xstep))
			...
			do timeindex = timestep-1, 0, -1
				do xindex = -xstep, xstep
					...
					do i = 1, k
						...
						call finalvalue(t(timeindex+1), x_point, yb, zb)
						expectyw = expectyw + w(i)*yb*sqrt(2*timesteplength)*a(i)
					end do 
					expectyw = expectyw/sqrt(pi)
					z(timeindex,xindex) = (expectyw)/(timesteplength)			
					call finalvalue(t(timeindex), x(xindex), y(timeindex, xindex),ztemp(timeindex, xindex))
				end do 
			end do
			!print*,"based on z ", z0
			!print*,"z(0,0)=",z(0,0)
