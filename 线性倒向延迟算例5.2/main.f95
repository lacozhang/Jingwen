...
! 	
	...
	! x space 
		...
		allocate(prev_z(0:timestep, -xstep:xstep))
		...
		do
			...
			do timeindex = timestep-1, 0, -1
				do xindex = -xstep, xstep
					...
					!call true_zvalue(t(timeindex), z(timeindex, xindex))
					y(timeindex,xindex) = expecty
					prev_z(timeindex, xindex) = z(timeindex, xindex)
					do j=1, timeindex-1
						y(timeindex, xindex) = y(timeindex, xindex) + 0.5*timesteplength*(prev_z(j, xindex) + prev_z(j+1, xindex))*timesteplength
					end do
			...
			if (firstcycle) then
				prev_y = y(0,0)
				firstcycle = .false.
			else if(abs(prev_y - y(0,0)) < 1e-8) then
