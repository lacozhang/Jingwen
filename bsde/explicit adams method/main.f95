...
! 	
		...
		real(kind=8):: expecty1, expectf1,expectf2
...
! when 0<= n <=N-1
		...
		! time fixed x change ,Y 	
			do xindex = -xstep ,xstep
				...
				expecty1 = expecty1/sqrt(pi)
				expectf1 = expectf1/sqrt(pi)
				expectf2 = expectf2/sqrt(pi)
			 	
				y(timeindex,xindex) = expecty1 + (3.0/2.0)*timesteplength*expectf1- 0.5*timesteplength*expectf2
				 if (timeindex > 0) then
