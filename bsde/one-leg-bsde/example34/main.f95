module main
	implicit none
	private
	public:: gauss_hermite, thelengthofx, bubble_sort, function34, finalvalue, newtoninterpl, leastsquare, li34
...
contains
	...
	subroutine li34(timestep, yerror, zerror, theta)
...
! 	
	...
	! last line of Y matrix and Z marix
		do xindex = -xstep, xstep
...
! when 0<= n <=N-2
		...
		do timeindex = timestep-1, 0, -1
			do xindex = -xstep, xstep
				...
				z(timeindex,xindex) = (expectyw+(1-theta)*timesteplength*expectfw-(1-theta)*timesteplength*expectz)/(theta*timesteplength)

				y0 = y(timeindex+1,xindex)
				do 
					call function34(y0,f)
					y1 = expecty + (1-theta)*timesteplength*expectf + theta*timesteplength*f
					diff=abs(y0-y1)
					if (diff <= 1e-10) then 
						exit
					end if
