...
contains
	...
	!xint returns result which is the length of x in positive
		...
		!print*,"a",a
		do i = 1, step
			xnext = x_max * x0 + (sqrt(8 * timesteplength) * a)
			!print*,"xnext=",xnext
...
! 	
		...
		real(kind=8):: expecty4, expectf1, expectf2, expectf3
	...
	! end GH
		...
		!print*,"xrange= ", xrange 
		xsteplength = timesteplength**(5.0/4.0)
	...
	!the start value of  Y matrix 
		do timeindex= timestep,timestep-3,-1
			do xindex = -xstep, xstep
...
! when 0<= n <=N-1
		...
		! time fixed x change ,Y 	
			do xindex = -xstep ,xstep
				...
				!calculate expecty4
				do i = 1,k
					x_point = x(xindex)+sqrt(8*timesteplength)*a(i)
					...
					yinterpl = (/y(timeindex+4,x_point1),y(timeindex+4,x_point2),y(timeindex+4,x_point3),&
								y(timeindex+4,x_point4)/)
					xinterpl = (/x(x_point1),x(x_point2),x(x_point3),x(x_point4)/)

					temp_point(1) = x_point
					temp_y=spline3(xinterpl, yinterpl, temp_point)
					yb = temp_y(1)
					expecty4 = expecty4 +w(i)*yb
					!calculate f1
					...
					call function34(yb,f2)
					expectf2 = expectf2 +w(i)*f2
					
					!calculate expectf3
					x_point = x(xindex)+sqrt(6*timesteplength)*a(i)
					x_point3 = floor(x_point/xsteplength)
					x_point2 = x_point3-1
					x_point1 = x_point3-2
					x_point4 = x_point3+1
					...
					!start to interpolate
					yinterpl = (/y(timeindex+3,x_point1),y(timeindex+3,x_point2),y(timeindex+3,x_point3),&
								y(timeindex+3,x_point4)/)
					temp_point(1) = x_point
					temp_y=spline3(xinterpl, yinterpl, temp_point)
					yb = temp_y(1)
					
					call function34(yb,f3)	
					expectf3 = expectf3 +w(i)*f3
		
				end do

				expecty4 = expecty4/sqrt(pi)
				expectf1 = expectf1/sqrt(pi)
				expectf2 = expectf2/sqrt(pi)
				expectf3 = expectf3/sqrt(pi)
			 	
				y(timeindex,xindex) = expecty4+ timesteplength*((8.0/3.0)*expectf1-(4.0/3.0)*expectf2+(8.0/3.0)*expectf3)
				if (timeindex > 0) then
