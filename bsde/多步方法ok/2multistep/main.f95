...
! 		
		...
		integer:: overflow
		real(kind=8):: f
		real(kind=8):: expecty2,expectfy1,expectfy2, expectfy3
		...
		! time fixed x change ,Y 	
			do xindex = -xstep ,xstep
				...
				do 
					...
					!y1 = expecty3 + 3*timesteplength*((1.0/8)*f+(3.0/8)*expectfy1+(3.0/8)*expectfy2+(1.0/8)*expectfy3)
					y1 = expecty2 + 2.0d0*timesteplength*((1.0d0/6.0d0)*f+(4.0d0/6.0d0)*expectfy1+(1.0d0/6.0d0)*expectfy2)
